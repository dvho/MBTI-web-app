...
function QuestionComponent (props) {
    return(
        <div className="question-block">
            ...
            <div style={{height: `${(props.item.choice * -1.5) + 75}px`, marginTop: `-${(props.item.choice * -1.5) + 75}px`, opacity: `${((((props.item.choice) * -1)+90)/200)}`}} className="graphs left-graph"></div>
            <div style={{height: `${(props.item.choice * 1.5) + 75}px`, marginTop: `-${(props.item.choice * 1.5) + 75}px`, opacity: `${(props.item.choice + 90)/200}`}} className="graphs right-graph"></div>
        </div>
    )
}
